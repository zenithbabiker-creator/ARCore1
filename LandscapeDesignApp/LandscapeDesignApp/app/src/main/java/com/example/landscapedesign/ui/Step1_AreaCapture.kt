...
fun Step1AreaCaptureScreen(
    ...
    // BUGFIX (blank-screen report): `LocalContext.current as? Activity` can be
    // null on some OEM ROMs / theme wrappers, because Compose's LocalContext
    // is sometimes a ContextThemeWrapper around the Activity rather than the
    // Activity itself. The previous `... ?: return` silently exited this
    // composable with NO log and NO crash — producing exactly a blank white
    // screen with nothing in Logcat, which matches the reported symptom.
    // We now unwrap any ContextWrapper chain to find the real Activity, and
    // log loudly if one truly can't be found instead of failing silently.
    val activity = remember(context) { context.findActivityOrNull() }
    if (activity == null) {
        android.util.Log.e("Step1AreaCapture", "No Activity found in LocalContext chain — cannot init ARCore. Context was: $context")
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text(
                text = stringResource(R.string.activity_context_error),
